...
class MyModel(pl.LightningModule):
    def __init__(self, conf: omegaconf.dictconfig.DictConfig):
        ...
        self.drop = torch.nn.Dropout(p=conf["model"]["model"]["dropout"])
        self.fc = torch.nn.LazyLinear(out_features=len(conf["other"]["labels"]))
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        ...
        .callbacks.BasePredictionWriter` should be used
    ...
    def configure_optimizers(self) -> Any:
        ...
        optimizer = torch.optim.AdamW(
            optimizer_grouped_parameters,
            lr=self.conf["model"]["train"]["learning_rate"],
        )
        lr_scheduler = transformers.get_cosine_schedule_with_warmup(
            optimizer,
            num_warmup_steps=self.conf["model"]["train"]["num_warmup_steps"],
            num_training_steps=self.conf["model"]["train"]["num_training_steps"],
        )
