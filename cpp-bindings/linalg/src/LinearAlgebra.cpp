...
double
LinearAlgebra::cosineBlas(const std::vector<std::vector<double>> &a,
                          const std::vector<std::vector<double>> &b) {
  size_t aRows = a.size();
  size_t aCols = a[0].size();
  size_t bRows = b.size();
  ...
  if (aCols != bCols and aRows != bRows) {
    throw std::runtime_error("The numbers of columns and rows of the 1st matrix must "
                             "equal the numbers of columns and rows of the 2nd matrix");
  }
  ...
  double dot_product = cblas_ddot(aCols * aRows, &a_flat[0], 1, &b_flat[0], 1);

  double a_norm = cblas_dnrm2(aCols * aRows, &a_flat[0], 1);
  double b_norm = cblas_dnrm2(bCols * bRows, &b_flat[0], 1);
